...
;( function( $ ) {
    $( function() {
        ...
        if ( envira_tabs_hash && envira_tabs_hash.indexOf( 'envira-tab-' ) >= 0 ) {
            ...
            if ( $( envira_tabs_current_tab ).find('#envira-config-custom-css-test').length > 0 && ( envira_css_editor === null ) ) {
                envira_css_editor = wp.codeEditor.initialize( $('#envira-config-custom-css-test'), editorSettings );
            }
        ...
        $( envira_tabs_nav ).on( 'click', '.nav-tab, a', function( e ) {
            ...
            if ( $( envira_tabs_section ).find( envira_tab ).find('#envira-config-custom-css-test').length > 0 && ( envira_css_editor === null ) ) {
                envira_css_editor = wp.codeEditor.initialize( $('#envira-config-custom-css-test'), editorSettings );
            }
