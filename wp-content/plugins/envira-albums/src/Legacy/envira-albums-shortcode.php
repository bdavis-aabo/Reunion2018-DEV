...
class Envira_Albums_Shortcode {
	...
	public function generate_album_item_markup( $album, $data, $id, $i ) {
		...
		if ( isset( $data['config']['display_titles'] ) && $data['config']['display_titles'] === 'above' && (int) $data['config']['columns'] !== 0 ) {
			$album_title = apply_filters( 'envira_albums_album_title', htmlspecialchars_decode( $item['title'] ), $id, $item, $data, $i );
		...
		if ( $this->is_mobile && ! envira_albums_get_config( 'lightbox_enabled', $data ) && ! envira_albums_get_config( 'mobile_lightbox', $data ) ) {
			$create_link = false;
		}
		...
		if ( $create_link ) {
			...
			$gallery_images = "data-gallery-images='" . envira_get_gallery_images( $id ) . "' ";

			$output .= '<a ' . $gallery_images . ' ' . $new_window . 'href="' . get_permalink( $id ) . '" ' . $gallery_images . ' class="envira-album-gallery-' . $id . ' envira-gallery-link" title="' . strip_tags( htmlspecialchars_decode( $item['title'] ) ) . '" ' . apply_filters( 'envira_gallery_output_link_attr', '', $id, $item, $data, $i ) . '>';
		...
		} elseif ( strpos( $item['cover_image_url'], 'cdninstagram' ) !== false ) {
		...
		if ( envira_albums_get_config( 'columns', $data ) == 0 ) {
			...
			// Automatic
			$output_item = '<img id="envira-gallery-image-' . sanitize_html_class( $id ) . '" class="envira-gallery-image envira-gallery-image-' . $i . $gallery_theme . ' ' . $envira_lazy_load . '" src="' . esc_url( $imagesrc ) . '"' . ( envira_albums_get_config( 'dimensions', $data ) ? ' width="' . envira_albums_get_config( 'crop_width', $data ) . '" height="' . envira_albums_get_config( 'crop_height', $data ) . '"' : ' height="' . $output_height . '" width="' . $output_width . '"' ) . ' data-envira-width="' . $output_width . '" data-envira-height="' . $output_height . '" data-envira-src="' . esc_url( $output_src ) . '" data-envira-caption="' . htmlentities( $item_caption ) . '" data-envira-item-id="' . $id . '" data-envira-album-id="' . $data['id'] . '" data-envira-gallery-id="' . sanitize_html_class( $id ) . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . strip_tags( htmlspecialchars_decode( $item['title'] ) ) . '" ' . apply_filters( 'envira_albums_output_image_attr', '', $item['cover_image_id'], $item, $data, $i ) . ' srcset="' . ( ( $envira_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $image_src_retina ) . ' 2x' ) . '" data-safe-src="' . ( ( $envira_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $output_src ) ) . '" />';
			...
			$output_item .= '<img id="envira-gallery-image-' . sanitize_html_class( $id ) . '" class="envira-gallery-image envira-gallery-image-' . $i . $gallery_theme . '" data-envira-index="' . $i . '" src="' . esc_url( $output_src ) . '"' . ( envira_albums_get_config( 'dimensions', $data ) ? ' width="' . envira_albums_get_config( 'crop_width', $data ) . '" height="' . envira_albums_get_config( 'crop_height', $data ) . '"' : '' ) . ' data-envira-src="' . esc_url( $output_src ) . '" data-envira-album-id="' . $data['id'] . '" data-envira-gallery-id="' . sanitize_html_class( $id ) . '" data-envira-item-id="' . $id . '" data-envira-caption="' . $item_caption . '" alt="' . esc_attr( $item['alt'] ) . '" title="' . strip_tags( htmlspecialchars( $item['title'] ) ) . '" ' . apply_filters( 'envira_albums_output_image_attr', '', $item['cover_image_id'], $item, $data, $i ) . ' data-envira-srcset="' . esc_url( $output_src ) . ' 400w,' . esc_url( $output_src ) . ' 2x" srcset="' . ( ( $envira_lazy_load ) ? 'data:image/gif;base64,R0lGODlhAQABAIAAAP///////yH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==' : esc_url( $image_src_retina ) . ' 2x' ) . '" />';
