					...
					foreach ( $albums as $album ) {
						if ( isset( $album['id'] ) ) {
							...
							if ( $album_id && $album['id'] == $album_id ) {
								echo '<option selected="selected" value="' . absint( $album['id'] ) . '">' . $title . '</option>';
							} else {
								echo '<option value="' . absint( $album['id'] ) . '">' . $title . '</option>';
							}
						}
					}
				}
			...
			<select id="<?php echo esc_attr( $this->get_field_id( 'envira_album_id' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'envira_album_id' ) ); ?>" style="width: 100%;">
				...
				if ( $albums && is_array( $albums ) ) {
					foreach ( $albums as $album ) {
						...
						$instance_envira_album_id = false;
						if ( isset( $instance['envira_album_id'] ) ) {
							$instance_envira_album_id = $instance['envira_album_id'];
						}

						echo '<option value="' . absint( $album['id'] ) . '"' . selected( absint( $album['id'] ), $instance_envira_album_id, false ) . '>' . $title . '</option>';
					}
				}
				?>
			</select> */
