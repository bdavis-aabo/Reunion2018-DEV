...
if ( ! class_exists( 'Envira_Videos_Shortcode' ) ) :
	class Envira_Videos_Shortcode {
		...
		public function load_css( $data ) {
			...
			wp_register_script( $this->base->plugin_slug . '-script', plugins_url( 'assets/js/envira-videos.js', $this->base->file ), array( 'jquery' ), $this->base->version, true );
		...
		public function change_gallery_output_before_image( $output, $id, $item, $data, $i ) {
			...
			if ( isset( $item['video'] ) && ( ! isset( $item['video_in_gallery'] ) || ! $item['video_in_gallery'] ) ) {
				...
				if ( Envira_Gallery_Shortcode::get_instance()->get_config( 'videos_play_icon', $data ) && $data['config']['columns'] != 0 ) {
					// Append the starting div
					// $output .= '<div class="envira-video-play-container">';
				}
			}
		...
		public function change_gallery_image( $output, $id, $item, $data, $i ) {
			...
			if ( isset( $item['video'] ) && ( ! isset( $item['video_in_gallery'] ) || ! $item['video_in_gallery'] ) ) {
				if ( Envira_Gallery_Shortcode::get_instance()->get_config( 'videos_play_icon', $data ) ) {
					...
					if ( $data['config']['columns'] == 0 || ( $data['config']['columns'] > 0 && ! $data['config']['lazy_loading'] ) ) {
		...
		video: {
			...
			playpause: <?php echo $instance->get_config( 'videos_playpause', $data ) ? $instance->get_config( 'videos_playpause', $data ) : 0; ?>,
			progress: <?php echo $instance->get_config( 'videos_progress', $data ) ? $instance->get_config( 'videos_progress', $data ) : 0; ?>,
			current: <?php echo $instance->get_config( 'videos_current', $data ) ? $instance->get_config( 'videos_current', $data ) : 0; ?>,
			duration: <?php echo $instance->get_config( 'videos_duration', $data ) ? $instance->get_config( 'videos_duration', $data ) : 0; ?>,
			volume: <?php echo $instance->get_config( 'videos_volume', $data ) ? $instance->get_config( 'videos_volume', $data ) : 0; ?>,
