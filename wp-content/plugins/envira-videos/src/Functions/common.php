...
function envira_video_get_video_type( $url, $item = false, $data = false, $type_only = false ) {
	...
	if ( preg_match( '%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $url, $y_matches ) ) {
		...
		// Some people use 'start' in the url(?) so switch this over to t
		...
		$start   = ( $seconds !== 0 ) ? '?start=' . $seconds : false;
		...
		if ( isset( $parts['fragment'] ) ) {
			...
			if ( array_key_exists( 't', $args ) ) {
				$args['start'] = ( $seconds !== 0 ) ? $seconds : false;
				unset( $args['t'] );
			}
		}
		...
		$parts    = explode( '/', $i_matches[0] );
		$video_id = $i_matches[2];
		// print_r ($url); print_r ($i_matches); exit;
		$type     = 'instagram';
		$mp4_link = false;
		...
		$embed_url = $url; // esc_url( add_query_arg( envira_video_get_twitch_args( $data ), '//player.twitch.tv/?video=' . $video_id ) );
	...
	// if ( $type == 'instagram' ) {  print_r ($result); exit; }
	return $result;
