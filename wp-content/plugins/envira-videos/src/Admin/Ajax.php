...
class Ajax {
	...
	public function get_remote_thumbnail( $video_src, $gallery_data, $video, $post_id ) {
		...
		if ( ! $attachment_id ) {
			...
			// Envira_Gallery_Import::get_instance()
		} else {
			$stream                  = array();
	...
	public function get_post_id_from_guid( $guid ) {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM $wpdb->postmeta WHERE meta_key='_wp_attached_file' AND meta_value=%s", $guid ) );
	}
	...
	public function get_instagram_thumbnail_url( $video_id, $instagram_type = 'p' ) {
		...
		// Determine video URL
		$prefix   = is_ssl() ? 'https' : 'http';
		$base_url = $prefix . '://www.instagram.com/' . $instagram_type . '/' . $video_id; // . '/media/';
		...
		$uri_parts = explode( '?', $thumbnail_url, 2 );

		if ( ! empty( $uri_parts[0] ) ) {

			return esc_url( $uri_parts[0] );
	...
	public function get_twitch_thumbnail_url( $video_id ) {
		...
		// Check request worked
		if ( $image_data_response == 404 || is_wp_error( $image_data ) || ! isset( $image_data['body'] ) ) {
			// Failed - fallback to SD Thumbnail
			return false;
		}
	 ...
	 * @return  array                   Gallery Data
	 */
	function save_bulk( $gallery_data, $meta, $attach_id, $post_id ) {
